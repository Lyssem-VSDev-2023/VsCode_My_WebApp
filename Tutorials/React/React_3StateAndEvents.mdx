...
# **React State and Events** <a id="react-state-and-events"></a><Link to="../tutorials/react">&#8678;</Link>
...
## State concept <a id="state-concept" />
...
## Add state to an application <a id="add-state-to-an-application" />
...
class Demo extends React.Component {
  ...
  render() {
    <button onClick={() => displayMessage("Clicked button!")}>
      Click me!
    </button>;
  }
}
...
## Update state <a id="update-state" />
...
## Add event handlers and update state <a id="add-event-handlers-and-update-state" />
